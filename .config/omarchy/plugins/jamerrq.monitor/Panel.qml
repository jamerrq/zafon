...
Panel {
  ...
  // Absolute path: actionProc runs the command without a shell, so nothing
  // expands ~ and nothing searches PATH.
  readonly property string toggleScript:
    Quickshell.env("HOME") + "/.config/yadm/scripts/toggle-monitor.sh"
  ...
  // Diverges from omarchy.monitor. Stock shells out to
  //   hyprctl keyword monitor <name>,disable
  // which Hyprland's Lua config parser rejects outright ("keyword can't work
  // with non-legacy parsers. Use eval."), while actionProc discards the exit
  // code -- so upstream a click silently does nothing, in both directions.
  // Tracked as omacom/omarchy#6968, unfixed as of 4.0.0.alpha.
  //
  // Routing through toggle-monitor.sh rather than inlining `hyprctl eval` here
  // buys the thing none of the upstream fixes address: the script also writes
  // the toggle flag under ~/.local/state/omarchy/toggles/hypr, so the change
  // survives a config reload and a reboot instead of being dropped the next
  // time hypr/monitors.lua's catch-all rule is re-applied. One code path for
  // the widget and for SUPER + ALT + U, so the two cannot disagree.
  function toggleDisplay(name, enabled) {
    if (!name) return
    if (enabled && root.enabledDisplayCount <= 1) return

    actionProc.command = [root.toggleScript, name]
