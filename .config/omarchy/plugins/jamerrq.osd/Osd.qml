...
Item {
  ...
  // it; the progress bar keeps the full gap -- its pill end recedes far less
  // than a letterform does, so it still needs the whole distance.
    ...
    BorderSurface {
      ...
      Row {
        ...
        Rectangle {
          ...
          // Pill ends, matching the rounded card and window borders. Keyed off
          // Style.cornerRadius (which mirrors Hyprland's decoration:rounding)
          // rather than hardcoded, so a square-cornered setup gets a square bar
          // -- the same way Ui/Toggle.qml and Ui/ToggleSwitch.qml decide.
          radius: Style.cornerRadius > 0 ? height / 2 : 0
          Rectangle {
            height: parent.height
            width: parent.width * (root.hasProgress ? root.value / root.maxValue : 0)
            color: Color.accent
            // Qt clamps radius to half the shorter side, so the fill stays a
            // clean lozenge as it shrinks instead of turning into a circle.
